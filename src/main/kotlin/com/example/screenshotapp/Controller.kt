...
class Controller {
    ...
    private lateinit var cutGC: GraphicsContext
    ...
    private var cropOnMouseDragged = EventHandler<MouseEvent> { e ->
        cutGC.fill = Color.BLACK
        cutGC.fillRect(0.0, 0.0, cutCanvas.width, cutCanvas.height)
        cutGC.fill = Color.WHITE
        cutGC.fillRect(cutX, cutY, e.x - cutX, e.y - cutY)
    }
    ...
    fun initialize() {
        ...
        cutGC = cutCanvas.graphicsContext2D
        ...
        if (!File(System.getenv("USERPROFILE") + "\\Desktop\\Screens\\" + "path.txt").exists()) {
            File(System.getenv("USERPROFILE") + "\\Desktop\\Screens").mkdir()
            File(System.getenv("USERPROFILE") + "\\Desktop\\Screens\\" + "path.txt").createNewFile()
        }
    }
    ...
    private fun saveToDirectory(path: String) {
        ...
        File(System.getenv("USERPROFILE") + "\\Desktop\\Screens\\" + "path.txt").writeText(finalPath)
    }
    ...
    private fun saveImage(fastSaveKey: Boolean) {
        var directory = File(System.getenv("USERPROFILE") +
                "\\Desktop\\Screens\\" + "path.txt").readText()
    ...
    private fun openImage() {
        val directory = File(System.getenv("USERPROFILE") + "\\Desktop\\Screens\\" + "path.txt").readText()
        val fileChooser = FileChooser()
        fileChooser.initialDirectory = File(directory)
